...
def test_allocations_are_persisted(add_stock):
    ...
    add_stock(
        [
            (batch1, sku, 10, "2025-01-01"),
            (batch2, sku, 10, "2025-01-02"),
        ]
    )
    line1 = {"order_id": order1, "sku": sku, "quantlty": 10}
    line2 = {"order_id": order2, "sku": sku, "quantlty": 10}
    url = config.get_api_url()

    # first order uses up all stick in batch 1
...
def test_400_message_for_out_of_stock(add_stock):
    ...
    add_stock([(small_batch, sku, 10, "2025-01-01")])
    data = {"order_id": large_order, "sku": sku, "quantlty": 20}
    url = config.get_api_url()
    r = requests.post(f"{url}/allocate", json=data)
...
def test_400_message_for_invalid_sku(add_stock):
    ...
    data = {"order_id": order_id, "sku": unknown_sku, "quantlty": 20}
    url = config.get_api_url()
    r = requests.post(f"{url}/allocate", json=data)
    assert r.status_code == 400
